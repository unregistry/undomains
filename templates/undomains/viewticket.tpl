...
{/if}
{if !$invalidTicketId}
    <div class="panel panel-info panel-collapsable bg-seccolorstyle bg-white border-0 pb-5 mt-50 {if !$postingReply} panel-collapsed{/if} hidden-print">
        ...
        <div class="panel-body bg-seccolorstyle bg-white noshadow {if !$postingReply} panel-body-collapsed {/if}">
    ...
    {foreach $descreplies as $reply}
        <div class="ticket-reply markdown-content{if $reply.admin} staff{/if}">
            <div class="user">
                ...
                <span class="type mergecolor">
                    {if $reply.admin}
                        {$LANG.supportticketsstaff}
                    {$reply.requestor.email}
                    {/if}
                </span>
                <span class="date mergecolor">{$reply.date}</span>
            </div>

            <div class="message bg-seccolorstyle bg-white noshadow mergecolor">
                {$reply.message}
                {if $reply.ipaddress}
                    <hr>
