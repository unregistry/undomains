...
<div class="tab-content">
    ...
    <div class="btn-group margin-bottom btn-domain-set">
                ...
                <ul class="dropdown-menu bg-seccolorstyle" role="menu">
                    <li><a href="#" id="nameservers" class="setBulkAction mergecolor"><i class="ico-globel"></i> {$LANG.domainmanagens}</a></li>
                    <li><a href="#" id="autorenew" class="setBulkAction mergecolor"><i class="ico-repeat"></i> {$LANG.domainautorenewstatus}</a></li>
                    <li><a href="#" id="reglock" class="setBulkAction mergecolor"><i class="ico-unlock"></i> {$LANG.domainreglockstatus}</a></li>
                    <li><a href="#" id="contactinfo" class="setBulkAction mergecolor"><i class="ico-user"></i> {$LANG.domaincontactinfoedit}</a></li>
                    {if $allowrenew}
                        ...
                        <tr onclick="clickableSafeRedirect(event, 'clientarea.php?action=domaindetails&amp;id={$domain.id}', false)">
                            ...
                            <td>
                                <div class="btn-group btn-group-sm" style="width:60px;">
                                    ...
                                    <ul class="dropdown-menu text-left dropdown-menu-right bg-seccolorstyle" role="menu">
                                        {if $domain.status eq 'Active'}
                                            <li><a class="mergecolor" href="clientarea.php?action=domaindetails&id={$domain.id}#tabNameservers"><i class="glyphicon glyphicon-globe"></i> {$LANG.domainmanagens}</a></li>
                                            <li><a class="mergecolor" href="clientarea.php?action=domaincontacts&domainid={$domain.id}"><i class="glyphicon glyphicon-user"></i> {$LANG.domaincontactinfoedit}</a></li>
                                            <li><a class="mergecolor" href="clientarea.php?action=domaindetails&id={$domain.id}#tabAutorenew"><i class="glyphicon glyphicon-globe"></i> {$LANG.domainautorenewstatus}</a></li>
                                            <li class="divider"></li>
                                        {/if}
                                        <li><a class="mergecolor" href="clientarea.php?action=domaindetails&id={$domain.id}"><i class="glyphicon glyphicon-pencil"></i> {$LANG.managedomain}</a></li>
                                        {if $allowrenew}
                                            {if $domain.canDomainBeManaged}
                                                <li><a class="mergecolor" href="{routePath('domain-renewal', $domain.domain)}"><i class="glyphicon glyphicon-refresh"></i> {lang key='domainsrenew'}</a></li>
                                            {else}
                                                <li class="disabled"><a href="#" onclick="return false;" class="disabled mergecolor" disabled="disabled"><i class="glyphicon glyphicon-refresh"></i> {lang key='domainsrenew'}</a></li>
                                            {/if}
