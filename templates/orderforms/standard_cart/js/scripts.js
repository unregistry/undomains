                ...
                WHMCS.http.jqClient.post(
                    ...
                    try {
                        ...
                        if (data && data.result) {
                            ...
                            // Detect premium/aftermarket domains.
                            // WHMCS returns these in several possible ways:
                            // 1. pricing is string "ContactUs"
                            // 2. pricing object where register value is "ContactUs"
                            // 3. No pricing at all (premium aftermarket with no standard price)
                            // 4. isValidDomain=false or error set
                            var isPremiumSuspect = false;
                            if (typeof domainResult !== 'string') {
                                if (typeof domainResult.pricing === 'string') {
                                    isPremiumSuspect = true;
                                ...
                                } else if (domainResult.pricing === undefined && !domainResult.error) {
                                    // No pricing returned at all but no error - likely premium aftermarket
                                    isPremiumSuspect = true;
                                }
                            }
