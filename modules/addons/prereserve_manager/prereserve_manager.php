<?php
/**
 * Pre-Reserve TLD Manager - WHMCS Addon Module
 * Manages pre-reserve TLDs (.fio, .degen, etc.)
 *
 * @package    PrereserveManager
 * @author     Undomains
 * @version    1.0.0
 */
...
function prereserve_manager_config()
{
    return [
        'name' => 'Pre-Reserve TLD Manager',
        'description' => 'Manage pre-reserve TLDs (.fio, .degen, etc.)',
        'version' => '1.0.0',
        'author' => 'Undomains',
        'language' => 'english',
        'fields' => [],
...
function prereserve_manager_activate()
{
    try {
        if (!Capsule::schema()->hasTable('mod_prereserve_tlds')) {
            Capsule::schema()->create('mod_prereserve_tlds', function ($table) {
                $table->increments('id');
            ...
            // Seed starter TLDs (moved from Unregistry Manager)
            Capsule::table('mod_prereserve_tlds')->insert([
                ['tld' => '.fio', 'extension' => '.fio', 'enabled' => 1, 'presale_mode' => 0, 'tld_mode' => 'coming_soon', 'display_order' => 1, 'register_price' => 0, 'transfer_price' => 0, 'renew_price' => 0, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s')],
                ['tld' => '.degen', 'extension' => '.degen', 'enabled' => 1, 'presale_mode' => 0, 'tld_mode' => 'coming_soon', 'display_order' => 2, 'register_price' => 0, 'transfer_price' => 0, 'renew_price' => 0, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s')],
            ]);
        }

        return ['status' => 'success', 'description' => 'Pre-Reserve TLD Manager activated successfully'];
    } catch (Exception $e) {
        return ['status' => 'error', 'description' => 'Error activating module: ' . $e->getMessage()];
    }
}

function prereserve_manager_deactivate()
{
    return ['status' => 'success', 'description' => 'Pre-Reserve TLD Manager deactivated. Data preserved.'];
}

function prereserve_manager_output($vars)
{
    $modulelink = $vars['modulelink'];

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        prereserve_manager_handlePost($modulelink);
    }

    prereserve_manager_pageTlds($modulelink, $vars);
}

function prereserve_manager_handlePost($modulelink)
{
    $action = $_POST['action'] ?? '';
    ...
    switch ($action) {
        case 'update_tld_mode':
            ...
            if ($tldId && in_array($mode, ['live', 'presale', 'reservation', 'coming_soon', 'disabled'])) {
                Capsule::table('mod_prereserve_tlds')->where('id', $tldId)->update([
                    'tld_mode' => $mode,
        ...
        case 'add_tld':
            ...
            if ($tld) {
                Capsule::table('mod_prereserve_tlds')->insert([
                    ...
                    'presale_mode' => 0,
                    'tld_mode' => $mode,
                    'display_order' => Capsule::table('mod_prereserve_tlds')->max('display_order') + 1,
        ...
        case 'delete_tld':
            $tldId = $_POST['tld_id'] ?? 0;
            if ($tldId) {
                Capsule::table('mod_prereserve_tlds')->where('id', $tldId)->delete();
            }
...
function prereserve_manager_pageTlds($modulelink, $vars)
{
    ...
    echo '<div class="prereserve-manager">';
    ...
    echo '<div class="col-sm-4"><input type="text" name="tld" class="form-control" placeholder="e.g., .fio" required></div></div>';
