<?php
/**
 * Web3 FIO Manager - WHMCS Addon Module
 * Manages FIO Protocol (.u displayed as @u) TLDs
 *
 * @package    Web3FioManager
 * @author     Undomains
 * @version    1.0.0
 */
...
function web3_fio_manager_config()
{
    return [
        'name' => 'Web3 (FIO) TLD Manager',
        'description' => 'Manage FIO Protocol (@u) TLDs',
        'version' => '1.0.0',
        'author' => 'Undomains',
        'language' => 'english',
        'fields' => [],
...
function web3_fio_manager_activate()
{
    try {
        if (!Capsule::schema()->hasTable('mod_web3_fio_tlds')) {
            Capsule::schema()->create('mod_web3_fio_tlds', function ($table) {
                $table->increments('id');
            ...
            // Seed starter TLD
            Capsule::table('mod_web3_fio_tlds')->insert([
                'tld' => '@u',
                'extension' => '.u',
                'enabled' => 1,
                'presale_mode' => 0,
        ...
        return ['status' => 'success', 'description' => 'Web3 (FIO) TLD Manager activated successfully'];
    } catch (Exception $e) {
        return ['status' => 'error', 'description' => 'Error activating module: ' . $e->getMessage()];
    }
}

function web3_fio_manager_deactivate()
{
    return ['status' => 'success', 'description' => 'Web3 (FIO) TLD Manager deactivated. Data preserved.'];
}

function web3_fio_manager_output($vars)
{
    $modulelink = $vars['modulelink'];

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        web3_fio_manager_handlePost($modulelink);
    }

    web3_fio_manager_pageTlds($modulelink, $vars);
}

function web3_fio_manager_handlePost($modulelink)
{
    $action = $_POST['action'] ?? '';
    ...
    switch ($action) {
        case 'update_tld_mode':
            ...
            if ($tldId && in_array($mode, ['live', 'presale', 'reservation', 'coming_soon', 'disabled'])) {
                Capsule::table('mod_web3_fio_tlds')->where('id', $tldId)->update([
                    'tld_mode' => $mode,
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
            }
        ...
        case 'add_tld':
            ...
            if (!$extension) {
                $extension = (strpos($tld, '.') === 0) ? $tld : '.' . $tld;
            }
            $mode = $_POST['tld_mode'] ?? 'coming_soon';
            ...
            if ($tld) {
                Capsule::table('mod_web3_fio_tlds')->insert([
                    ...
                    'presale_mode' => 0,
                    'tld_mode' => $mode,
                    'display_order' => Capsule::table('mod_web3_fio_tlds')->max('display_order') + 1,
        ...
        case 'delete_tld':
            $tldId = $_POST['tld_id'] ?? 0;
            if ($tldId) {
                Capsule::table('mod_web3_fio_tlds')->where('id', $tldId)->delete();
            }
...
function web3_fio_manager_pageTlds($modulelink, $vars)
{
    $updated = $_GET['updated'] ?? null;
    $added = $_GET['added'] ?? null;
    $deleted = $_GET['deleted'] ?? null;

    $tlds = Capsule::table('mod_web3_fio_tlds')
        ->orderBy('display_order')
        ->get();

    echo '<div class="web3-fio-manager">';
    ...
    echo '<div class="form-group"><label class="col-sm-2 control-label">Display TLD *</label>';
    echo '<div class="col-sm-4"><input type="text" name="tld" class="form-control" placeholder="e.g., @u" required></div></div>';

    echo '<div class="form-group"><label class="col-sm-2 control-label">Extension</label>';
    echo '<div class="col-sm-4"><input type="text" name="extension" class="form-control" placeholder="e.g., .u (auto-filled if empty)"></div></div>';
    ...
    echo '<div class="panel-body">';
    echo '<table class="table table-striped"><thead><tr><th>Display TLD</th><th>Extension</th><th>Status</th><th>Mode</th><th>Register</th><th>Transfer</th><th>Renew</th><th>Actions</th></tr></thead><tbody>';
    ...
    foreach ($tlds as $tld) {
        ...
        echo '<tr>';
        echo '<td><input type="text" name="tld" class="form-control input-sm" value="' . htmlspecialchars($tld->tld) . '" style="width:60px"></td>';
        echo '<td><input type="text" name="extension" class="form-control input-sm" value="' . htmlspecialchars($tld->extension) . '" style="width:60px"></td>';
        ...
        echo '</tr>';
        echo '</form>';
        echo '<tr><td colspan="8" style="padding:0;border:none">';
