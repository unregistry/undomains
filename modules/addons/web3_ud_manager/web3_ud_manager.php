<?php
/**
 * Web3 UD Manager - WHMCS Addon Module
 * Manages Unstoppable Domains (.u) TLDs
 *
 * @package    Web3UdManager
 * @author     Undomains
 * @version    1.0.0
 */
...
function web3_ud_manager_config()
{
    return [
        'name' => 'Web3 (UD) TLD Manager',
        'description' => 'Manage Unstoppable Domains (.u) TLDs',
        'version' => '1.0.0',
        'author' => 'Undomains',
        'language' => 'english',
        'fields' => [],
...
function web3_ud_manager_activate()
{
    try {
        if (!Capsule::schema()->hasTable('mod_web3_ud_tlds')) {
            Capsule::schema()->create('mod_web3_ud_tlds', function ($table) {
                $table->increments('id');
            ...
            // Seed starter TLD
            Capsule::table('mod_web3_ud_tlds')->insert([
                'tld' => '.u',
                'extension' => '.u',
                'enabled' => 1,
                'presale_mode' => 0,
        ...
        return ['status' => 'success', 'description' => 'Web3 (UD) TLD Manager activated successfully'];
    } catch (Exception $e) {
        return ['status' => 'error', 'description' => 'Error activating module: ' . $e->getMessage()];
    }
}

function web3_ud_manager_deactivate()
{
    return ['status' => 'success', 'description' => 'Web3 (UD) TLD Manager deactivated. Data preserved.'];
}

function web3_ud_manager_output($vars)
{
    $modulelink = $vars['modulelink'];

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        web3_ud_manager_handlePost($modulelink);
    }

    web3_ud_manager_pageTlds($modulelink, $vars);
}

function web3_ud_manager_handlePost($modulelink)
{
    $action = $_POST['action'] ?? '';
    ...
    switch ($action) {
        case 'update_tld_mode':
            ...
            if ($tldId && in_array($mode, ['live', 'presale', 'reservation', 'coming_soon', 'disabled'])) {
                Capsule::table('mod_web3_ud_tlds')->where('id', $tldId)->update([
                    'tld_mode' => $mode,
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
            }
        ...
        case 'add_tld':
            ...
            if ($tld) {
                Capsule::table('mod_web3_ud_tlds')->insert([
                    ...
                    'presale_mode' => 0,
                    'tld_mode' => $mode,
                    'display_order' => Capsule::table('mod_web3_ud_tlds')->max('display_order') + 1,
        ...
        case 'delete_tld':
            $tldId = $_POST['tld_id'] ?? 0;
            if ($tldId) {
                Capsule::table('mod_web3_ud_tlds')->where('id', $tldId)->delete();
            }
        ...
        case 'toggle_enabled':
            $tldId = $_POST['tld_id'] ?? 0;
            $tld = Capsule::table('mod_web3_ud_tlds')->where('id', $tldId)->first();
            if ($tld) {
                Capsule::table('mod_web3_ud_tlds')->where('id', $tldId)->update([
                    'enabled' => $tld->enabled ? 0 : 1,
                    'updated_at' => date('Y-m-d H:i:s'),
                ]);
            }
            header("Location: $modulelink&updated=1");
...
function web3_ud_manager_pageTlds($modulelink, $vars)
{
    $updated = $_GET['updated'] ?? null;
    $added = $_GET['added'] ?? null;
    $deleted = $_GET['deleted'] ?? null;

    $tlds = Capsule::table('mod_web3_ud_tlds')
        ->orderBy('display_order')
        ->get();

    echo '<div class="web3-ud-manager">';
